...
# myproject.include_router(file.router, prefix='/v1/file', tags=['临时文件管理'])
app.include_router(doc_parsing.router, prefix='/v1/unstructured', tags=['非结构化文档解析'])
# myproject.include_router(auth.router, prefix='/v1/auth', tags=['身份认证'])
app.include_router(doc_parsing.router, prefix='/v1/other', tags=['其他'])

if __name__ == '__main__':
    uvicorn.run('main:myproject', host='0.0.0.0', port=8000, reload=True, workers=1)
